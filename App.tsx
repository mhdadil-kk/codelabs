...
const MonacoEditor = React.memo(({
  ...
  useEffect(() => {
    ...
    const loadMonaco = () => {
      window.require(['vs/editor/editor.main'], () => {
        ...
        if (window.monaco) {
          // Disable heavy semantic validation for smoother typing
          ...
          if (monacoLang === 'javascript') {
            jsDefaults.setCompilerOptions({
              ...
              checkJs: false, // Performance optimization
            });
          }
        }
        ...
        // Debounced change handler to prevent high-frequency React state updates
        ...
        // Remeasure fonts to ensure cursor alignment
        setTimeout(() => {
  ...
  useEffect(() => {
    if (editorRef.current) {
      ...
      // Only update if significantly different to avoid cursor jumps / conflict with typing
...
export default function App() {
  ...
  useEffect(() => {
    // Only reset code if it's the very first load or explicit change, 
    // but here we just respect the current language state.
    // We could persist code too if needed, but for now just language code.
    setCode(INITIAL_CODE[language]);
    setOutput([]);
    if (language === 'react') {
      const doc = generateReactTemplate(INITIAL_CODE['react']);
      setReactSrcDoc(doc);
    }
  }, [language]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (resizingDir === 'horizontal' && containerRef.current) {
        const containerRect = containerRef.current.getBoundingClientRect();
        const newWidth = e.clientX - containerRect.left;
        let newRatio = (newWidth / containerRect.width) * 100;
        newRatio = Math.min(60, Math.max(20, newRatio));
        setSplitRatio(newRatio);
      } else if (resizingDir === 'vertical' && editorContainerRef.current) {
        const containerRect = editorContainerRef.current.getBoundingClientRect();
        const newHeight = e.clientY - containerRect.top;
        let newRatio = (newHeight / containerRect.height) * 100;
        newRatio = Math.min(85, Math.max(15, newRatio));
        setVerticalSplit(newRatio);
      }
    };
    const handleMouseUp = () => setResizingDir(null);
  ...
  const getAI = () => {
    ...
    if (!key) {
      setOutput(p => [...p, "Error: Missing API Key."]);
      return null;
    }
  ...
  const generateQuiz = async () => {
    ...
    try {
      const res = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: `Generate 3 hard ${language} multiple choice questions. Return JSON format: [{question, options:[], correctIndex}]`,
        config: { responseMimeType: 'application/json' }
      });
      setQuizQuestions(JSON.parse(res.text || "[]"));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const renderChat = () => (
    <div className="flex flex-col h-full bg-[#131314] relative">
      {/* Header */}
      <div className="h-20 border-b border-[#333] flex items-center justify-between px-6 bg-[#131314] shrink-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="CodeLab" className="h-16 object-contain" />
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-[#1E1F20] border border-[#333]">
  ...
  const renderEditor = () => (
    <div className="flex-1 flex flex-col h-full bg-[#1E1E1E] relative" ref={editorContainerRef}>
      {/* Toolbar - Standard Dark */}
      <div className="h-20 border-b border-[#333] flex items-center justify-between px-6 bg-[#131314]">
        <LanguageSelector current={language} onChange={setLanguage} />
        <div className="flex items-center gap-3">
          <button
            onClick={generateQuiz}
            className="flex items-center gap-2 text-gray-400 hover:text-white px-3 py-2 rounded-lg hover:bg-[#2A2B2D] transition-colors"
          >
          ...
          <button
            onClick={runCode}
            disabled={isRunning}
            className="flex items-center gap-2 bg-[#4285F4] hover:bg-[#3b77db] text-white px-5 py-2 rounded-full font-medium shadow-lg hover:shadow-blue-900/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {/* Vertical Resizer */}
      <div
        className="h-2 bg-[#131314] hover:bg-[#4285F4] cursor-row-resize flex items-center justify-center transition-colors z-10 border-y border-[#333] group"
        onMouseDown={(e) => startResizing(e, 'vertical')}
      >
        <GripHorizontal className="w-4 h-4 text-gray-500 group-hover:text-white" />
      </div>

      {/* Output / Preview Area (Resizable Bottom) */}
      <div className="flex-1 bg-[#1E1E1E] overflow-hidden flex flex-col min-h-0">
        {language === 'react' ? (
    ...
    <div className="flex-1 bg-[#1E1E1E] p-8 flex flex-col items-center justify-center">
      <div className="max-w-2xl w-full">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setViewMode('editor')} className="text-gray-400 hover:text-white">Exit</button>
        </div>

        {quizQuestions.length > 0 && currentQuizIndex < quizQuestions.length ? (
          <div className="bg-[#2A2B2D] p-8 rounded-2xl border border-[#333] shadow-xl">
            <div className="mb-6">
              ...
              <h3 className="text-xl text-white mt-2 font-medium leading-relaxed">{quizQuestions[currentQuizIndex].question}</h3>
            </div>
            <div className="space-y-3">
              {quizQuestions[currentQuizIndex].options.map((opt, idx) => (
                <button
                  key={idx}
                  onClick={() => {
                    if (idx === quizQuestions[currentQuizIndex].correctIndex) setQuizScore(s => s + 1);
          ...
          <div className="text-center bg-[#2A2B2D] p-12 rounded-2xl border border-[#333]">
            <Flame className="w-16 h-16 text-orange-500 mx-auto mb-4" />
            <h3 className="text-2xl text-white font-bold mb-2">Quiz Complete!</h3>
            <p className="text-gray-400 mb-6">You scored {quizScore} out of {quizQuestions.length}</p>
            <button
              onClick={() => { setViewMode('editor'); setCurrentQuizIndex(0); setQuizScore(0); }}
              className="bg-[#4285F4] text-white px-8 py-3 rounded-full font-medium hover:bg-[#3367d6] transition-colors"
            >
              Return to Coding
            </button>
          </div>
  ...
  return (
    <div ref={containerRef} className="flex h-screen bg-[#131314] overflow-hidden font-sans">
      {/* Chat Panel (Resizable) */}
      <div style={{ width: `${splitRatio}%` }} className="h-full shrink-0 bg-[#131314] flex flex-col border-r border-[#333]">
        {renderChat()}
      </div>

      {/* Resizer Handle */}
      <div
        className="w-1 h-full bg-[#131314] hover:bg-[#4285F4] cursor-col-resize z-20 flex flex-col justify-center items-center group transition-colors"
        onMouseDown={(e) => startResizing(e, 'horizontal')}
      >
        <div className="w-0.5 h-8 bg-[#444] group-hover:bg-[#888] rounded-full transition-colors" />
      </div>
